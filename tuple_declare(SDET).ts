/*  Tuple - an immutable data type which can be considered an "array" with a defined number of elements, means it includes multiple values of different data types thus representing heterogenous collection of values.      
...
// declaring typescript tuple in 2 ways, way 1 (in which we write a simple Javscript array of mixed types which is nothing but a typescript tuple declared without types)
...
console.log(empDetails_2);                  // prints [200, 'Shagird', 65000, true]
console.log('\n===========================================');
...
const last : any = empDetails_2.pop();
...
for(const row of stu){
    for(const elem of row) console.log(elem);     // prints 6 rita 9 sarah 13 vesper (vertical order)
}
